...
module.exports = (router) => {
  const limitApiRate = (requests, durationInMinutes) =>
    rateLimit({
  ...
  router.get(
    "/company/:stocksymbol/balancesheet",
  ...
  router.get(
    "/globalmarket/status",
    globalMarketStatusController.getGlobalMarketStatus
  );
  router.get(
    "/news/sentiments",
  ...
  router.get(
    "/ticker/:stocksymbol/search",
    tickerSearchController.getTickerSearch
  );
  router.get(
    "/timeseries/:stocksymbol/daily",
    timeSeriesController.getDailyTimeSeries
  );
  router.get(
    "/timeseries/:stocksymbol/intraday",
    timeSeriesController.getIntradayTimeSeries
  );
  router.get(
    "/top/gainers/loosers/traded",
    topGainerAndLooserController.getTopGainerLooserAndTraded
  );
  router.post(
    "/user/signup",
    limitApiRate(5, 15),
