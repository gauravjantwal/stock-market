...
function Allwatchlist() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (response1?.data) {
        ...
        if (getwatchlists?.length > 0 && watchlistid == "") {
      ...
      if (topGainersLosers && getwatchlists?.length > 0) {
        const response = await WatchlistService.GetBookmark(
          getwatchlists[0]._id
        );
        if (response?.data) {
          let result = topGainersLosers?.top_gainers?.filter((o1) =>
            response?.data.some((o2) => o1.ticker === o2)
          );
          settopgainersdata(topGainersLosers?.top_gainers);
          setBookmark(result);
        }
  ...
  const handleDelete = async (stocksymbol) => {
    const response = await WatchlistService.DeleteBookmark(
      watchlistid,
      stocksymbol
    );
    const response1 = await WatchlistService.GetBookmark(watchlistid);
    let result = topgainersdata?.filter((o1) =>
      response1?.data.some((o2) => o1.ticker === o2)
    );
    setBookmark(result);
            ...
            <>
              <div className="mt-3">
                {watchlist?.map((element) => (
                  <button
                    className={
                      watchlistid == element._id
                        ? "btn  text-color-green rounded-pill fw-bold ml-2 shadow btn-bg-color-green text-white"
                        : "btn btn-light text-color-green rounded-pill fw-bold ml-2 shadow"
                    }
                    key={element?._id}
                    type="button"
                      ...
                      <tr key={row.ticker}>
                        ...
                        <td>
                          <button
                            ...
                            className="btn btn-light text-color-green rounded-pill shadow fw-bold"
                          >
                            Delete
                          </button>
                        </td>
        ...
        <div>
          <Modal show={show} onHide={handleClose} backdrop="static">
            ...
            <Modal.Footer>
              ...
              <Button
                variant="primary"
                className="btn btn-bg-color-green text-white"
                onClick={handleSave}
              >
                Save
              </Button>
